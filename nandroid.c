...
static int tar_compress_wrapper(const char* backup_path, const char* backup_file_image, int callback) {
    ...
    if (strcmp(backup_path, "/data") == 0 && (volume_for_path("/sdcard") == NULL || ignore_data_media))
      sprintf(tmp, "cd $(dirname %s) ; tar cpvf %s.tar --exclude 'media' $(basename %s) ; exit $?", backup_path, backup_file_image, backup_path);
    else
      sprintf(tmp, "cd $(dirname %s) ; tar cpvf %s.tar $(basename %s) ; exit $?", backup_path, backup_file_image, backup_path);
...
int nandroid_backup_partition_extended(const char* backup_path, const char* mount_point, int umount_when_finished) {
    ...
    if (0 != ret) {
        ui_print("Error making a backup image of %s!\n", mount_point);
        return ret;
    }
    return 0;
}
...
int nandroid_backup_partition(const char* backup_path, const char* root) {
    ...
    if (strcmp(vol->fs_type, "mtd") == 0 ||
        ...
        if (0 != (ret = backup_raw_partition(vol->fs_type, vol->device, tmp))) {
            ui_print("Error backing up %s image!", name);
            return ret;
        }
        return 0;
    }
...
static int tar_extract_wrapper(const char* backup_file_image, const char* backup_path, int callback) {
    ...
    char tmp[PATH_MAX];
    if (strcmp(backup_path, "/data") == 0 && (volume_for_path("/sdcard") == NULL || ignore_data_media))
        sprintf(tmp, "cd $(dirname %s) ; tar xpvf - --exclude 'media' < %s ; exit $?", backup_path, backup_file_image);
    else
        sprintf(tmp, "cd $(dirname %s) ; tar xpvf - < %s ; exit $?", backup_path, backup_file_image);
    ...
    if (restore_wimax && vol != NULL && 0 == stat(vol->device, &s))
    {
        ...
        else
        {
            ...
            if (0 != (ret = format_volume("/wimax")))
                return print_and_error("Error formatting wimax!\n");
